...
E_exc = 5.0    # bridge excitation (V)
GF    = 2.0    # gauge factor (full bridge)
# ----------------------------------------------------
# REMOVE OFFSET USING REAL BASELINE
# ----------------------------------------------------
baseline = df["volts"].iloc[0:200].mean()
df["volts_corr"] = df["volts"] - baseline

# SIGN FIX
df["Vr"] = baseline - df["volts"]

# ----------------------------------------------------
#. STRAIN FROM AMPLIFIED SIGNAL
# ----------------------------------------------------
df["strain"] = df["Vr"] / (GAIN * GF * E_exc)
...
# ----------------------------------------------------
# K-MEANS PLATEAU DETECTION (FLAT 10% SAMPLES)
# ----------------------------------------------------

moment_vals = df["moment_smooth"].values
deriv = np.abs(np.gradient(moment_vals))

# Select the flattest 10% of samples (removes spikes + transitions)
...
# Use median for robust plateau value
...
# Assign plateaus
M_0g, M_6g, M_12g, M_18g, M_24g, M_30g, M_36g = cluster_medians_sorted

measured_moment = np.array([
    M_0g, M_6g, M_12g, M_18g, M_24g, M_30g, M_36g
])

# ----------------------------------------------------
# ANALYTICAL MOMENT
# ----------------------------------------------------
loads_g = np.array([0, 6, 12, 18, 24, 30, 36])
...
plt.tight_layout()
plt.show()
plt.savefig("FullBridgeCalCurve.png")
# ----------------------------------------------------
...
plt.tight_layout()
plt.show()
plt.savefig("fullBridgeMomentPlateau.png")
# ----------------------------------------------------
